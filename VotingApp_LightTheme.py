...
class VotingApp(QMainWindow):
    ...
    def check_pass(self, ok: str):
        if self.admin_password.text() == "avadmingsc":
            if ok == "results":
                ...
                self.admin_screen()
        elif self.admin_password.text() == "about":
            self.about_screen()
        else:
            self.home_screen()
    ...
    def about_screen(self):
        ...
        start_button = QPushButton("Go Back")
        start_button.setFont(QFont("Arial", 14))
        start_button.clicked.connect(self.home_screen)
        layout.addWidget(start_button, alignment=Qt.AlignmentFlag.AlignCenter)
    ...
    def admin_screen(self):
        ...
        self.role_input.setPlaceholderText("Enter role and press Enter to move to name")
        self.role_input.returnPressed.connect(lambda: self.contestant_input.setFocus())

        self.contestant_input = QLineEdit()
        self.contestant_input.setPlaceholderText("Enter contestant name and press Enter to add")
        self.contestant_input.returnPressed.connect(self.add_contestant)
        ...
        reset_btn.clicked.connect(self.confirm)
